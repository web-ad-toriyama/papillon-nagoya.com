...
if ($posts['confirm']) {
    if ($posts['method'] === 'url') {
        ...
        //拡張子の確認
        if (!stristr($path, ".jpg") && !stristr($path, ".jpeg") && !stristr($path, ".gif")) {
            error("送信できるのはJPEG、GIF形式のファイルのみです。");
        }
    ...
    // バナー画像・動画はリサイズしない
    if ($params['field'] == 'banner' || $params['field'] == 'movie') {
        $result = true;
    } else {
...
function checkExt()
{
    ...
    if ($params['field'] == 'movie') {
        ...
        if (!stristr($_FILES['image_f']['name'], ".jpg") && !stristr($_FILES['image_f']['name'], ".jpeg") && !stristr($_FILES['image_f']['name'], ".gif")) {
            error("送信できるのはJPEG、GIF形式のファイルのみです。");
        }
    }
}
